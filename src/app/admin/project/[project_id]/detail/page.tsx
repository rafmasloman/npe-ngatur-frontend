...
import TaskAdminPanel from '../../../../../features/projects/project_detail/Tab/Task/components/TaskAdminPanel';
...
const ProjectAdminDetail = () => {
  ...
  const tess = projectDetail?.data?.project.member?.map((m, index) => {
    if (index < 2) {
      return m;
    }
  });
  ...
  console.log('project : ', projectDetail.data);

  if (!projectDetail.data && !milestonesProject.data) {
    return (
    ...
    <Container size={'xl'}>
      <Stack className="container mx-auto" gap={0}>
        <Group
          ...
          align="center"
          className="p-5 rounded-3xl rounded-bl-none border-b-0 bg-white rounded-br-none border border-neutral-200 "
        >
          <Stack gap={20}>
            <Group>
              <Avatar
        ...
        <BaseTab>
          <Tabs.Panel value="overview" className="pt-12">
            <OverviewPanel
              options={{ isProjectManager: false }}
              ...
              client={projectDetail.data?.project?.client}
            />
          </Tabs.Panel>
