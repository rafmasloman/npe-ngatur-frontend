...
interface IProjectAdminCardPropsType {
  controller?: {
    onDelete?: ReactNode;
    onEdit?: ReactNode;
  };
  menu?: MenuProps;
  data: IProjectResponseApi;
}
...
const ProjectAdminCard = ({
  ...
  return (
    <>
      ...
      <Link href={`${PROJECTS_ADMIN_PAGE}/${data.id}/detail`}>
        <Card
          ...
          >
            <Group justify="space-between" align="center">
              {!data.projectIcon ? (
                ...
                <Avatar
                  src={`${process.env.NEXT_PUBLIC_API_DOWNLOAD_FILES_URL}/projects/${data.projectIcon}`}
                  alt="Project Logo"
                  className="h-fit"
                  size={36}
                  radius={0}
              ...
              <Grid mt={25}>
                {data.platform?.split(',').map((platform, index) => {
                  return (
                    <Grid.Col
                      span={data.platform?.split(',').length > 1 ? 6 : 12}
                      key={index}
                    >
                      <Box
            ...
            <Stack>
              ...
              <Text>
                ...
                <span style={{ color: COLORS.success }}>
                  {filterCompletedTask(data.task)?.length}
                </span>
                /{data.task?.length}
              </Text>
            </Stack>
